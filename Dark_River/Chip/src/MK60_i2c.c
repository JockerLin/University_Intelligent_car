...
/*!
 *  @brief      读取I2C设备指定地址寄存器的数据（8位数据）
 *  @param      I2Cn_e        I2C模块(I2C0、I2C1)
 *  @param      SlaveID     从机地址(7位地址) 前7位为从机地址，第8位为读写位，其中0表示写，1表示读。
 *  @param      reg         从机寄存器地址
 *  @return                 读取的寄存器值
 *  @since      v5.0
 *  Sample usage:       uint8 value = i2c_read_reg(I2C0, 0x1D, 1);
 */
uint8 i2c_read_reg(I2Cn_e i2cn, uint8 SlaveID, uint8 reg)//读八位的
{
...
/*!
 *  @brief      读取I2C设备指定地址寄存器的数据（16位数据） 还在改
 *  @param      I2Cn_e        I2C模块(I2C0、I2C1)
 *  @param      SlaveID     从机地址(7位地址) 前7位为从机地址，第8位为读写位，其中0表示写，1表示读。
 *  @param      reg         从机寄存器地址
 *  @return                 读取的寄存器值
 ...
 *  Sample usage:       uint8 value = i2c_read_reg(I2C0, 0x1D, 1);
 */
uint16 i2c_read_reg_16bit(I2Cn_e i2cn, uint8 SlaveID, uint8 reg)//读16位的
{
    ...
    result1 = I2C_D_REG(I2CN[i2cn]);                     //虚假读取一次，启动接收数据
    i2c_Wait(i2cn);                                     //等待接收完成
    
    result1 = I2C_D_REG(I2CN[i2cn]);                     //读取数据
    i2c_Wait(i2cn);                                     //等待接收完成

    i2c_Stop(i2cn);                                     //发送停止信号
    
    result2 = I2C_D_REG(I2CN[i2cn]);                     //读取数据
    //i2c_Wait(i2cn);                                     //等待接收完成
    
    i2c_delay();                                        //必须延时一下，否则出错

    return ((((int16)result1)<<8)|result2);
    
}

/*!
 *  @brief      读取I2C设备指定地址寄存器的数据（32位数据） 还在改
 *  @param      I2Cn_e        I2C模块(I2C0、I2C1)
 *  @param      SlaveID     从机地址(7位地址) 前7位为从机地址，第8位为读写位，其中0表示写，1表示读。
 *  @param      reg         从机寄存器地址
 *  @return                 读取的寄存器值
 ...
 *  Sample usage:       uint8 value = i2c_read_reg(I2C0, 0x1D, 1);
 */
void i2c_read_reg_32bit(I2Cn_e i2cn, uint8 SlaveID, uint8 reg)//读32位的
{

    //先写入寄存器地址,再读取数据,因此此过程是 I2C 的复合格式,改变数据方向时需要重新启动
    uint8 result1,result2,result3,result4;
    ...
    result3 = I2C_D_REG(I2CN[i2cn]);                     //读取数据
    i2c_Wait(i2cn);                                     //等待接收完成
    
    i2c_Stop(i2cn);                                     //发送停止信号
    
    result4 = I2C_D_REG(I2CN[i2cn]);                     //读取数据
    i2c_Wait(i2cn);                                     //等待接收完成
